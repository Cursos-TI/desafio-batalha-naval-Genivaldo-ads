    ...
    for (int i = 0; i < 3; i++) {           // altura do cone
        for (int j = -i; j <= i; j++) {     // largura varia conforme i
            if (linha + i < TAM && coluna + j >= 0 && coluna + j < TAM) {
                tabuleiro[linha + i][coluna + j] = '*'; // marca o ataque
            }
        }
    }
}
...
void habilidadeCruz(char tabuleiro[TAM][TAM], int linha, int coluna) {
    for (int i = -2; i <= 2; i++) {
        if (linha + i >= 0 && linha + i < TAM)
            tabuleiro[linha + i][coluna] = '*'; // vertical

        if (coluna + i >= 0 && coluna + i < TAM)
            tabuleiro[linha][coluna + i] = '*'; // horizontal
    }
}
...
int main() {
    ...
    habilidadeCone(tabuleiro, 1, 3); //representa o cone
    habilidadeCruz(tabuleiro, 5, 5);  // representa a cruz
    habilidadeOctaedro(tabuleiro, 7, 7);   //representa o octaedro

    // Exibe o resultado final
    imprimirTabuleiro(tabuleiro);

    return 0; //fim do pgrograma
}
